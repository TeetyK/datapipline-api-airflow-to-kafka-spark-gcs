"""Unit Tests สำหรับฟังก์ชัน fetch_api_data"""
import pytest
import requests
import json
...
class TestFetchApiData:
    ...
    def test_fetch_missing_api_token_logs_warning(
        self, mock_env_vars, caplog
    ):
    ...
    def test_fetch_handles_single_object_response(
        ...
        with patch("dags.api.requests.get") as mock_get:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = single_user
            mock_get.return_value = mock_response
            ...
            with patch("dags.api.os.getenv", side_effect=mock_env_vars()):
                ...
                # Assert: ควรแปลงเป็น list ที่มี 1 item
